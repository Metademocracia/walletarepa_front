      ...
      <aside class="container-available">
        ...
        <div class="divcol center" style="margin-top: 25px;">
          <div clasS="d-flex align-center" style="gap: 10px">
            <h3 class="mb-0">SALDO DISPONIBLE</h3>
            <v-icon size="16" @click="offuscate = !offuscate">{{ offuscate ? 'mdi-eye-off' : 'mdi-eye'}}</v-icon>
          </div>
          <img src="../assets/sources/icons/warning-orange.svg"  alt="warning icon" style="--w: 25px; --h: 25px; margin-top: 6px" @click="$refs.modalCryptos.model = true">
        </div>
      </aside>
            ...
            // if(items.actions.length === 1) {
              ...
              switch (items.first_action_type) {
                case "transfer": // "TRANSFER":
                  typeParam = dataTransfer.direction === "send" ? "sent" : "receive"; // items.predecessor_account_id === wallet ? "sent" : "receive";
                  accountParam = dataTransfer.direction === "send" ? dataTransfer.receiver : dataTransfer.sender;
                  amountParam = (dataTransfer.direction === "send" ? "-" : "+")+Number(dataTransfer.amount).toFixed(5);
                ...
                case "createAccount": // "CREATE_ACCOUNT":
                  typeParam = "account";
                  accountParam = items.receiver; // items.receiver_account_id;
                  break;
                case "addKey": // "ADD_KEY":
                  typeParam = "access";
                  accountParam = items.receiver; // items.receiver_account_id;
                ...
                case "functionCall": // "FUNCTION_CALL":
                  typeParam = "function";
                  accountParam = items.receiver; // items.receiver_account_id;
                  text2 = items.method_name; // items.actions[0].method;
                  break;
                default:
                  typeParam = "access";
                  accountParam = items.receiver; // items.receiver_account_id;
